	...
	for scene_name, scene in scene_lib.items():
		for shot in scene:
			...
			for action in shot.actions:
				...
				if shot.scale != '' and shot.scale is not None:
					action_shot_dict[action_name].append(shot.scale)

	with open('shot_scale.txt', 'w') as sst:
		sst.write('shot_name \t num_appearances\n')
		for shot_name, nums in shot_scale_dict.items():
			sst.write('{}\t{}\n'.format(str(shot_name), str(nums)))
	with open('action_shot_dict.txt', 'w') as asd:
		for action, shot_types in action_shot_dict.items():
			asd.write(str(action) + '\t' + '\t'.join(str(item) for item in shot_types))
			asd.write('\n')

	with open('action_shot_dict.txt', 'w') as asd:
		for action, shot_types in action_shot_dict.items():
			asd.write(str(action) + '\t' + str([shot_types]))
			asd.write('\n')
